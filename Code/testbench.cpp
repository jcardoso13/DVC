...
int main(int argc, char **argv)
{
    ...
    for (j = 0; j < nSTAGE; j++)
    {
        ...
        for (i = 0; i < 25; i++)
        {
            ...
            versat.stage[j].memA[0].write(i, pixels[25 * j + i]);
        }
        ...
        for (i = 0; i < 9; i++)
        {
            ...
            versat.stage[j].memA[1].write(i, weights[9 * j + i]);
        }
        ...
        {
            bias = rand() % 20 - 10;
            versat.stage[j].memA[1].write(9, bias);
        }
    }
    end = clock();
    ...
    for (i = 0; i < nSTAGE; i++)
    {
        ...
        versat.stage[i].memA[0].setIter(3);
        versat.stage[i].memA[0].setIncr(1);
        versat.stage[i].memA[0].setDelay(delay);
        versat.stage[i].memA[0].setPer(3);
        versat.stage[i].memA[0].setDuty(3);
        versat.stage[i].memA[0].setShift(5 - 3);

        //configure mem1A to read kernel
        versat.stage[i].memA[1].setIter(1);
        versat.stage[i].memA[1].setIncr(1);
        versat.stage[i].memA[1].setDelay(delay);
        versat.stage[i].memA[1].setPer(10);
        versat.stage[i].memA[1].setDuty(10);

        //configure muladd0
        versat.stage[i].muladd[0].setSelA(sMEMA[0]);
        versat.stage[i].muladd[0].setSelB(sMEMA[1]);
        versat.stage[i].muladd[0].setFNS(MULADD_MACC);
        versat.stage[i].muladd[0].setIter(1);
    ...
    versat.stage[nSTAGE - 1].memA[2].setIter(1);
    versat.stage[nSTAGE - 1].memA[2].setIncr(1);
    versat.stage[nSTAGE - 1].memA[2].setDelay(MEMP_LAT + 8 + MULADD_LAT + ALULITE_LAT + delay);
    versat.stage[nSTAGE - 1].memA[2].setPer(1);
    versat.stage[nSTAGE - 1].memA[2].setDuty(1);
    versat.stage[nSTAGE - 1].memA[2].setSel(sALULITE[0]);
    versat.stage[nSTAGE - 1].memA[2].setInWr(1);
    end = clock();
    ...
    for (i = 0; i < 3; i++)
    {
        for (j = 0; j < 3; j++)
        {
            ...
            for (k = 0; k < nSTAGE; k++)
                versat.stage[k].memA[0].setStart(i * 5 + j);
            versat.stage[nSTAGE - 1].memA[2].setStart(i * 3 + j);
    ...
    {
        for (j = 0; j < 3; j++)
            printf("%d\t", (int16_t)versat.stage[nSTAGE - 1].memA[2].read(i * 3 + j));
        printf("\n");
    }
    ...
    versat.stage[0].memB[1].setStart(9);
    versat.stage[0].memB[1].setIter(9);
    versat.stage[0].memB[1].setPer(9);
    versat.stage[0].memB[1].setDuty(9);

    for (i = 0; i < nSTAGE; i++)
    {

        //configure mem0A to read all 3x3 blocks from feature map
        versat.stage[i].memA[0].setIter2(3);
        versat.stage[i].memA[0].setPer2(3);
        versat.stage[i].memA[0].setShift2(5 - 3);
        versat.stage[i].memA[0].setIncr2(1);
        versat.stage[i].memA[0].setStart(0);

        //configure mem1A to read kernel
        versat.stage[i].memA[1].setIter(9);
        versat.stage[i].memA[1].setIncr(1);
        versat.stage[i].memA[1].setDelay(delay);
        versat.stage[i].memA[1].setPer(9);
        versat.stage[i].memA[1].setDuty(9);
        versat.stage[i].memA[1].setShift(-9);
    ...
    versat.stage[nSTAGE - 1].memA[2].setStart(10);
    versat.stage[nSTAGE - 1].memA[2].setIter(9);
    versat.stage[nSTAGE - 1].memA[2].setIncr(1);
    versat.stage[nSTAGE - 1].memA[2].setDelay(MEMP_LAT + 8 + MULADD_LAT + ALULITE_LAT + delay);
    versat.stage[nSTAGE - 1].memA[2].setPer(9);
    versat.stage[nSTAGE - 1].memA[2].setDuty(1);
    versat.stage[nSTAGE - 1].memA[2].setSel(sALULITE[0]);
    versat.stage[nSTAGE - 1].memA[2].setInWr(1);
    end = clock();
    ...
    {
        for (j = 0; j < 3; j++)
            printf("%d\t", (int16_t)versat.stage[nSTAGE - 1].memA[2].read(i * 3 + j + 10));
        printf("\n");
    }
    print_versat_info();
    //clear conf_reg of VERSAT 0
    versat.stage[0].clearConf();

#ifdef CONF_MEM_USE
    //store conf_reg of VERSAT 1 in conf_mem (addr 0)
    versat.stage[1].confMemWrite(0);
#endif
...
int main(int argc, char **argv)
{
    ...
    for (j = 0; j < nSTAGE; j++)
    {
        ...
        versat.stage[j].vi[1].setExtIncr(1);
        versat.stage[j].vi[1].setExtShift(0);
        ...
        if (j == 0)
        {
            ...
            versat.stage[j].vi[2].setExtIncr(1);
    ...
    versat.stage[0].vi[2].setIntIter(9);
